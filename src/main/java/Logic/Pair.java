...
public class Pair {
    ...
    public Pair(String pathBackCover, String pathFrontCover, Display display){
        ...
        // Array represents the cards which are together
    ...
    public static void compareCards(){
        ...
        for(Pair pair : allPairs) {
            if (pair.card1.getId() == SELECTED_CARDS[0].getId() && pair.card2.getId() == SELECTED_CARDS[1].getId()) {
                SELECTED_CARDS[0].setCardInvisible();
                SELECTED_CARDS[1].setCardInvisible();
                resetSelectedCards();
                break;
            } else if (pair.card1.getId() == SELECTED_CARDS[1].getId() && pair.card2.getId() == SELECTED_CARDS[0].getId()) {
                SELECTED_CARDS[0].setCardInvisible();
                SELECTED_CARDS[1].setCardInvisible();
                resetSelectedCards();
                break;
